...
- **Final-validation warning baseline** (task 07): Full-solution clean build succeeds with 0 errors and ~152 analyzer warnings (WFO1000, SYSLIB0057/0027/0060, CS0618/0672/8073, CA1845/1864/1865/1866, WFDEV005). These are pre-existing, repo-wide diagnostics driven by the intentionally aggressive `AnalysisMode=all` / `AnalysisLevel=preview-all` config in `targets.props` — not migration regressions. User approved (Option A) accepting them as the baseline rather than reopening completed tier tasks. Task 07 removed 24 MSB3884 (orphaned `AllRules.ruleset`) and 2 NU1510 (redundant `System.Configuration.ConfigurationManager` / `System.Net.Http` package refs).
- **Deferred: Central Package Management (CPM)** — per upgrade options, CPM (Directory.Packages.props) was deferred to post-migration. Not applied; recommended as a follow-up to centralize the per-project package versions.
- **Deferred: Nullable reference types** — per upgrade options, NRTs left disabled across all projects. Recommended as a separate opt-in modernization effort.
- **No test projects** — solution contains no unit/integration test projects (no Microsoft.NET.Test.Sdk / xUnit / NUnit / MSTest references); validation is build-based only.
