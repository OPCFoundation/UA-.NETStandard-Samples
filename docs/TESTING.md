...
| AlarmCondition | The configured area tree, areas as notifiers of the server, alarms travelling from source to area, and an alarm which leaves a client's where clause reported one last time with Retain false | `AlarmConditionClientModelTests` |
| HistoricalAccess | Raw reads, continuation points, read-at-time, aggregates, inserting and deleting with the modified history remembering it - and saying what was done, when and by whom - annotations read and written, an atomic batch rolled back as a whole, an unsupported aggregate refused before the provider, the audit event of a replace and a delete carrying the values they displaced, a live variable historized by the capture pipeline without a subscriber, a client write captured the same way, the installed HistoricalDataConfiguration object, structured history with several entries per instant, and which items are still being collected | `HistoricalAccessNodeManagerTests` |
...
| AlarmCondition | The configured area tree, areas as notifiers of the server, alarms travelling from source to area, a condition refresh replaying the retained dialogs, and a suppressed alarm leaving the list without a refresh | `AlarmConditionClientModelTests` |
| HistoricalAccess | Raw reads, continuation points, modified reads with their modification info, the configuration and the capabilities the server reports, the audit events of history updates arriving while watching, and which items are still being collected | `HistoricalAccessClientModelTests` |
| HistoricalEvents | The well tree, event history page by page through the history client, a report written, rewritten field by field and deleted through the model, and the two refusals the sample declares | `HistoricalEventsClientModelTests` |
...
| TestData | Static write round trip, simulated values while monitored, which single variable is archived, the archive read back over an authenticated session, and a read at a time and an average which run on the framework fallbacks because the provider implements raw reads only |
