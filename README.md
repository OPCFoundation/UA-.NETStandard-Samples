...
| [RuntimeNodeSets Server](Workshop/RuntimeNodeSets/Server) | **Server** | Hosts a vendor NodeSet2 document with no source generation and no node manager class, and adds, reloads and removes that model through `INodeManagerLifecycle` while the server runs. See the [sample README](Workshop/RuntimeNodeSets/README.md). |
| [RuntimeNodeSets Client](Workshop/RuntimeNodeSets/Client) | **Client** | Loads, reloads and removes the model of its server over OPC UA, and watches what each of the three reload modes does to a MonitoredItem. |
